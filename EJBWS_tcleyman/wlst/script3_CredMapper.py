...
def function_CredentialMapper():
    '''
    function_CredentialMapper(): configure a SAML CredentialMapper
    Note after this function the Server must be restarted
    '''
    try:
        edit()
        startEdit()    
        cd('/SecurityConfiguration/domainNEWJMSClient/Realms/myrealm')
        cm = cmo.createCredentialMapper('SAML_tcleyman_CredMapperV2', 'com.bea.security.saml2.providers.SAML2CredentialMapper')
        cm.setIssuerURI('http://tomstest/')
        print 'save and activating changes ....'
        save()
        activate(block="true")
        return 'success'   
...
print function_CredentialMapper()
